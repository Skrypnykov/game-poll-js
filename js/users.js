...
export async function register(userData) {
  ...
  apiPost(fullUrl, userData)
    .then((responseData) => {
      ...
      signIn(userData);
      setTimeout(
        () => (statusTextRegister.innerText = "Пароль не менше 8 символів."),
        3000
      );
    })
    .catch((error) => {
  ...
  apiPost(fullUrl, userData)
    .then((responseData) => {
      if (responseData.email) {
        ...
        userInfo.innerHTML = "Вітаємо, " + "<a href='/pages/account.html'>" + responseData.nickname + "</a>"; 
        setTimeout(() => $("#modalSignIn").modal("hide"), 1000);
        setTimeout(() => $("#modalInfo").modal("show"), 1500);
        setLocalData(responseData);
        setSignOut();
      } else {
        statusTextSignIn.innerText = "Невірні дані";
      }
      setTimeout(
        () => (statusTextSignIn.innerText = "Введіть реєстраційні дані."),
        3000
      );
    })
    .catch((error) => {
...
export async function verifyAuth() {
  ...
  if (!res.ok) {
    ...
    setLocalData(body);
    setSignOut();
    userInfo.innerHTML = "Вітаємо, " + "<a href='/pages/account.html'>" + body.nickname + "</a>"; 
  }
}
  ...
  apiPost(fullUrl, {})
    .then((responseData) => {
      if (responseData) {
        ...
        setTimeout(
          () =>
            (statusTextRecovery.innerText = "Введіть адресу електронної пошти"),
          3000
        );
      }
    })
